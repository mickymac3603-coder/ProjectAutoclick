...
class AutoClickerApp:
    ...
    def __init__(self):
        ...
        self.key_seq        = []      # ปุ่มที่กดวน (Loop)
        self.hold_seq       = []      # ปุ่มที่กดค้างไว้ตลอด (Hold)
        self.capturing_key  = False
        self.capture_target = 'loop'  # 'loop' หรือ 'hold'
    ...
    def _build_ui(self):
        ...
        self.speed_lbl.pack()
        speed_row = tk.Frame(sp)
        speed_row.pack(pady=(2, 0))
        tk.Label(speed_row, text='ครั้ง/วินาที (1-1000):').pack(side='left', padx=(0, 6))
        speed_vcmd = (root.register(self._validate_speed_input), '%P')
        self.speed_spin = tk.Spinbox(speed_row, from_=1, to=1000, width=6,
                                      textvariable=self.cps, font=('Arial', 11, 'bold'),
                                      justify='center', command=self._refresh_speed_lbl,
                                      validate='key', validatecommand=speed_vcmd)
        self.speed_spin.pack(side='left')
        self.speed_spin.bind('<Return>', self._on_speed_entry)
        self.speed_spin.bind('<FocusOut>', self._on_speed_entry)
    ...
    def _build_mode2_tab(self, nb):
        ...
        # ── กล่องที่ 1: ปุ่มที่กดวน (Loop) ──
        loop_box = tk.LabelFrame(f, text=' ปุ่มที่กดวน (Loop) ', padx=6, pady=6)
        loop_box.pack(fill='x', pady=(0, 6))
        seq_wrap = tk.Frame(loop_box, bg='#e8e8e8', relief='sunken', bd=1, height=36)
        seq_wrap.pack(fill='x', pady=(0, 4))
        seq_wrap.pack_propagate(False)
        self.seq_frame = seq_wrap
        loop_btn_row = tk.Frame(loop_box)
        loop_btn_row.pack(fill='x')
        tk.Button(loop_btn_row, text='+ เพิ่มปุ่ม',
                  command=lambda: self._capture_key_start('loop')).pack(side='left', padx=2)
        tk.Button(loop_btn_row, text='− ลบสุดท้าย',
                  command=lambda: self._remove_last_key('loop')).pack(side='left', padx=2)
        tk.Button(loop_btn_row, text='ล้างทั้งหมด',
                  command=lambda: self._clear_keys('loop')).pack(side='left', padx=2)
        self.loop_hint_lbl = tk.Label(loop_box, text='', fg='#0066cc', font=('Arial', 9, 'italic'))
        self.loop_hint_lbl.pack(pady=(2, 0))

        # ── กล่องที่ 2: ปุ่มที่กดค้างไว้ (Hold) ──
        hold_box = tk.LabelFrame(f, text=' 📌 ปุ่มที่กดค้างไว้ (Hold) ', padx=6, pady=6)
        hold_box.pack(fill='x')
        hold_wrap = tk.Frame(hold_box, bg='#fff2cc', relief='sunken', bd=1, height=36)
        hold_wrap.pack(fill='x', pady=(0, 4))
        hold_wrap.pack_propagate(False)
        self.hold_frame = hold_wrap
        hold_btn_row = tk.Frame(hold_box)
        hold_btn_row.pack(fill='x')
        tk.Button(hold_btn_row, text='+ เพิ่มปุ่มค้าง',
                  command=lambda: self._capture_key_start('hold')).pack(side='left', padx=2)
        tk.Button(hold_btn_row, text='− ลบสุดท้าย',
                  command=lambda: self._remove_last_key('hold')).pack(side='left', padx=2)
        tk.Button(hold_btn_row, text='ล้างทั้งหมด',
                  command=lambda: self._clear_keys('hold')).pack(side='left', padx=2)
        self.hold_hint_lbl = tk.Label(hold_box, text='', fg='#0066cc', font=('Arial', 9, 'italic'))
        self.hold_hint_lbl.pack(pady=(2, 0))

        tk.Label(f, text='💡 ปุ่มใน "Hold" จะถูกกดค้างไว้ตลอดขณะทำงาน\n'
                          '   ปุ่มใน "Loop" จะถูกกดวนซ้ำไปเรื่อย ๆ\n'
                          '   เช่น Hold = Ctrl, Loop = Enter → ค้าง Ctrl ค้างไว้แล้วกด Enter ซ้ำ',
                 fg='gray', font=('Arial', 8), justify='left').pack(anchor='w', pady=(6, 0))
    ...
    def _start_kb_listener(self):
        def on_press(key):
            ...
            if self.capturing_key:
                ...
                if target == 'hold':
                    self.hold_seq.append(item)
                else:
                    self.key_seq.append(item)
                self.capturing_key = False
                self.root.after(0, lambda: [
                    self.loop_hint_lbl.config(text=''),
                    self.hold_hint_lbl.config(text=''),
                    self._refresh_seq_display(),
                    self._refresh_hold_display()
                ])
                return
    ...
    def _validate_speed_input(self, proposed):
        # อนุญาตให้เว้นว่างชั่วคราวตอนพิมพ์ใหม่ และต้องเป็นตัวเลขเท่านั้น
        return proposed == '' or proposed.isdigit()

    def _on_speed_entry(self, _event=None):
        try:
            val = int(self.cps.get())
    ...
    def _capture_key_start(self, target='loop'):
        self.capturing_key  = True
        self.capture_target = target
        if target == 'hold':
            self.hold_hint_lbl.config(text='กดปุ่มที่ต้องการเพิ่ม (Hold)…')
        else:
            self.loop_hint_lbl.config(text='กดปุ่มที่ต้องการเพิ่ม (Loop)…')

    def _remove_last_key(self, target='loop'):
        if target == 'hold':
            if self.hold_seq:
                self.hold_seq.pop()
                self._refresh_hold_display()
    ...
    def _clear_keys(self, target='loop'):
        if target == 'hold':
            self.hold_seq.clear()
            self._refresh_hold_display()
        else:
    ...
    def _refresh_seq_display(self):
        ...
        if not self.key_seq:
            ...
            return
        for i, item in enumerate(self.key_seq):
            if i:
                tk.Label(self.seq_frame, text='→', bg='#e8e8e8', fg='gray').pack(side='left')
            tk.Label(self.seq_frame, text=f" {item['label']} ", bg='#d0d0d0', relief='raised',
    ...
    def _refresh_hold_display(self):
        for w in self.hold_frame.winfo_children():
            w.destroy()
        if not self.hold_seq:
            tk.Label(self.hold_frame, text='(ว่าง)', fg='gray', bg='#fff2cc').pack(side='left', padx=8, pady=4)
            return
        for i, item in enumerate(self.hold_seq):
            if i:
                tk.Label(self.hold_frame, text='+', bg='#fff2cc', fg='gray').pack(side='left')
            tk.Label(self.hold_frame, text=f"📌 {item['label']} ", bg='#ffcc66', relief='raised',
                     font=('Consolas', 10, 'bold'), padx=4, pady=2).pack(side='left', padx=2, pady=3)
    ...
    def _start(self):
        self._on_speed_entry()
        mode = self.mode.get()
        if mode == 2 and not self.key_seq and not self.hold_seq:
            self.loop_hint_lbl.config(text='⚠ กรุณาเพิ่มปุ่ม Loop หรือ Hold อย่างน้อย 1 ปุ่มก่อน', fg='red')
            return
    ...
    def _click_loop(self):
        ...
        if mode == 1:
            ...
            hold_items = list(self.hold_seq)
            tap_items  = list(self.key_seq)
            held_keys  = []
            try:
                # กดค้างปุ่มในกล่อง Hold ไว้ตั้งแต่เริ่ม (เช่น Ctrl) แล้วไม่ปล่อยจนกว่าจะหยุด
                for item in hold_items:
                    try:
                        self.keyboard_ctrl.press(item['value'])
                        held_keys.append(item['value'])
                    except Exception:
                        pass
                ...
                if tap_items:
                    idx = 0
                    while self.running:
                        press_val = tap_items[idx % len(tap_items)]['value']
                        try:
                    ...
                    # มีแต่ปุ่ม Hold ไม่มีปุ่มกดวน แค่ค้างไว้จนกว่าจะหยุด
                    while self.running:
                        time.sleep(0.05)
            finally:
                for k in held_keys:
